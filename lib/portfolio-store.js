...
export function normalizePortfolioContent(content) {
  ...
  next.heroSubtitle = typeof next.heroSubtitle === "string" && next.heroSubtitle.trim()
    ? next.heroSubtitle
    : portfolioContent.heroSubtitle || defaultHeroSubtitle;
  next.marqueeItems = withArray(next.marqueeItems, portfolioContent.marqueeItems || defaultMarqueeItems);
  next.makerSignals = withArray(next.makerSignals, portfolioContent.makerSignals);
  next.metrics = withArray(next.metrics, portfolioContent.metrics).map((metric) => ({
    value: metric.value || "",
    label: metric.label || "",
  }));
